...
class MenuPrincipal():
    def __init__(self, juego):
        ...
        btn = DirectButton(text = 'Jugar', command = juego.jugar, pos = (0, 0, 0.2), parent = self.menu_inicio, scale = 0.1,
                        frameSize = (-4, 4, -1, 1), text_scale = 0.75, text_pos = (0, -0.2))
        btn.setTransparency(True)

        btn = DirectButton(text = 'Opciones', command = juego.opciones, pos = (0, 0, -0.2), parent = self.menu_inicio, scale = 0.1, 
                        frameSize = (-4, 4, -1, 1), text_scale = 0.75, text_pos = (0, -0.2))
        btn.setTransparency(True)

        btn = DirectButton(text = 'Salir', command = juego.salir, pos = (0, 0, -0.6), parent = self.menu_inicio, scale = 0.1, 
                        frameSize = (-4, 4, -1, 1), text_scale = 0.75, text_pos = (0, -0.2))
        btn.setTransparency(True)
...
class MenuOpciones():
    def __init__(self, juego):
        ...
        DirectLabel(text = 'Resolución', scale = 0.1, pos = (-.5, 0, 0.6), parent = self.menu_opciones, text_fg = (1, 1, 1, 1), 
                    frameSize = (-4, 4, -1, 1), text_pos = (0, -0.2))

        btn = DirectOptionMenu(text="Resolución de pantalla", pos = (.5, 0, 0.6), parent = self.menu_opciones, scale=0.1, initialitem=0,
        ...
        DirectLabel(text = 'Modo de pantalla', scale = 0.1, pos = (-.5, 0, 0.2), parent = self.menu_opciones, text_fg = (1, 1, 1, 1), 
                    frameSize = (-4, 4, -1, 1), text_pos = (0, -0.2))
        ...
        DirectRadioButton(text='Pantalla Completa', variable=[1], value=[0], indicatorValue=False ,scale=0.08, pos=(0.5, 0, .23), command=juego.modo_pantalla, 
                          parent=self.menu_opciones, extraArgs=[True]),
        DirectRadioButton(text='Modo ventana', variable=[1], value=[1], indicatorValue=False, scale=0.08, pos=(0.43, 0, .10), command=juego.modo_pantalla, 
                          parent=self.menu_opciones, extraArgs=[False])
        ]
        ...
        self.volumen = DirectSlider(range=(0,100), value=100, pos = (.5, 0, -0.2), pageSize=3, command=juego.musica, scale=0.5, parent=self.menu_opciones)


        btn = DirectButton(text = 'Volver al menú', command = juego.menu, pos = (0, 0, -0.6), parent = self.menu_opciones, scale = 0.1, 
                        frameSize = (-4, 4, -1, 1), text_scale = 0.75, text_pos = (0, -0.2))
        btn.setTransparency(True)
